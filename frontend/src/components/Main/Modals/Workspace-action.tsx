...
const WorkspaceActionModal = () => {
  ...
  const { setWorkspaceActionToggle, workspaceActionToggle } = useAppStore();
  const { selectedWorkspace, workspaces, setWorkspaces } = useWorkspaceStore();
  ...
  const navigateAfterDelete = async () => {
    try {
      ...
      console.log(error);
      toast.error("Failed to navigate to a workspace");
    }
  };
  ...
  const handleAction = async () => {
    if (action === "delete") {
      try {
        ...
        navigateAfterDelete();
        setWorkspaceActionToggle(false)
